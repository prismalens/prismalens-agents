...
export class DeepAgentsPlugin implements AgentPlugin {
	readonly name = "deepagents-cli";
	readonly processName = "deepagents";
	...
	getLaunchCommand(task: AgentTask, _config: AgentLaunchConfig): string[] {
		...
		const cmd = ["deepagents", "-n", taskContent];

		if (this.config) {
			const allowList = buildAllowList(this.config, "comma");
			cmd.push("--shell-allow-list", allowList);
		}
	...
	async isAvailable(): Promise<boolean> {
		const result = await checkCommand("deepagents");
